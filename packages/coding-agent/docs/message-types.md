...
`details` is tool-specific. `addedToolNames` records tools that became available at this point in the transcript. Optional `usage` reports nested model work performed by the tool and contributes to full-session statistics, but it is not part of the main model-call usage.
