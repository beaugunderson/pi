...
Register every tool first, keep optional tools inactive, and use `pi.setActiveTools()` from a loader tool to add matching tools. Purely additive changes expose the new definitions on the next model request. Other active-set changes use the complete current tool list instead.

Anthropic Sonnet, Opus, and Fable 4.5 or newer, Fireworks Messages, and OpenAI `gpt-5.4` or newer can load added definitions at the tool-result position without changing the initial tool-schema prefix. On Fireworks, name the loader `ToolSearch` or `tool_search` to preserve that prefix. Other models receive the definitions through the normal active tool list.

For verified custom endpoints, native handling can be enabled with `compat.supportsToolReferences` for Anthropic Messages or `compat.supportsToolSearch` for OpenAI Responses. Leave these disabled unless the endpoint and model support the corresponding protocol.
